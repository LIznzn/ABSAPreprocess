...
def semeval15_parser(file_path, term_type="aspect", rm_conflict_senti=True):
...
if __name__ == "__main__":
    ...
    file_map = {
        "restaurant": {
            "train": ("datasets/SemEval2015/train/ABSA-15_Restaurants_Train_Final.xml", "aspect"), # join/aspect
            "test": ("datasets/SemEval2015/test/ABSA15_Restaurants_Test.xml", "aspect") # join/aspect
        },
        "hodel": {
            "test": ("datasets/SemEval2015/test/ABSA15_Hotels_Test.xml", "aspect") # join/aspect
        }
    }

    for dataset, dataset_value in file_map.items():
        for train_test, file_param_value in dataset_value.items():
            datas = semeval15_parser(file_path=file_param_value[0], term_type=file_param_value[1])
            out_path = "datasets/SemEval2015/mydata/{}_{}_{}_{}.csv".format(dataset, train_test, file_param_value[1], len(datas))
